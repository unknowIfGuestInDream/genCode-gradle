...
public abstract class AbstractGenProcedureModel extends AbstractFreeMarkerTemplate implements InitializingBean {
	 ...
	 * @param result 返回结果集
	 */
	protected void dealCommonCode(Configuration configuration, Map<String, Object> objectMap,
			Map<String, Object> result) {
		// tab页集合, 保证返回顺序
		List<String> list = new ArrayList<>(
				Arrays.asList("controller", "service", "serviceImpl", "repository", "BaseUtils", "ProcedureUtils"));
		result.put("BaseUtils", FreeMarkerUtil.getTemplateContent(configuration, objectMap, "common/BaseUtils.ftl"));
		result.put("ProcedureUtils",
				FreeMarkerUtil.getTemplateContent(configuration, objectMap, "common/ProcedureUtils.ftl"));
	 ...
	 */
	protected void dealProcParams(Map<String, Object> map, StringJoiner inParams, StringJoiner outParams,
			StringJoiner procedureParams, StringJoiner serviceNote, StringJoiner repositoryParam,
			StringJoiner repositoryResult, NameConventService nameConvent, DataBaseProcedureService dbProcedure) {
		if ("IN".equals(map.get("IN_OUT")) || "false".equals(map.get("IN_OUT").toString())) {
			...
			if ("DATE".equals(dataType)) {
				repositoryParam.add("            statement.set" + dbProcedure.getRepositoryOutTypeCode(dataType) + "(\""
			...
			String dataType = map.get("DATA_TYPE").toString();
			procedureParams.add(":" + value + "");
			repositoryParam.add("            statement.registerOutParameter(\"" + value + "\", "
					+ dbProcedure.getRepositoryOutType(dataType) + ");");
			if ("REF CURSOR".equals(dataType)) {
				repositoryResult.add("            returnValue.put(\"" + nameConvent.getResultName(value)
						+ "\", ProcedureUtils.resultHash((ResultSet) cs.get"
						+ dbProcedure.getRepositoryOutTypeCode(dataType) + "(\"" + value + "\")));");
			}
			else {
				repositoryResult.add("            returnValue.put(\"" + nameConvent.getResultName(value) + "\", cs.get"
						+ dbProcedure.getRepositoryOutTypeCode(dataType) + "(\"" + value + "\"));");
			}
		}
	...
	protected Map<String, Object> dealProcdure(String moduleName, String packageName, String userName,
		...
		for (int i = 0; i < length; i++) {
			...
			for (Map<String, Object> map : list) {
				dealProcParams(map, inParams, outParams, procedureParams, serviceNote, repositoryParam,
						repositoryResult, nameConvent, dbProcedure);
			}
	 ...
	 * @param configuration ftl模板引擎配置
	 */
	public Map<String, Object> genCode(String moduleName, String packageName, String userName,
			List<String> procedureNameList, String author, NameConventService nameConvent,
			DataBaseProcedureService dbProcedure, DataSourceUtil dataSourceUtil, Configuration configuration) {
		Map<String, Object> result = new HashMap<>(16);
		// 方法代码
		Map<String, Object> objectMap = dealProcdure(moduleName, packageName, userName, procedureNameList, nameConvent,
				dbProcedure, dataSourceUtil, configuration);
		objectMap.put("package", packageName);
		objectMap.put("module", moduleName);
