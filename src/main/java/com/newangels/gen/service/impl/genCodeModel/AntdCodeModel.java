...
public class AntdCodeModel extends AbstractGenCodeModel {
	...
	protected void dealCommonCode(String tableName, String tableDesc, String moduleName, String moduleDesc,
		...
		super.dealCommonCode(tableName, tableDesc, moduleName, moduleDesc, packageName, author, hasDelBatch, hasExport,
	...
	protected void dealOtherCode(String tableName, String tableDesc, String moduleName, String moduleDesc,
		...
		dealUpdateForm(updateForm, moduleName, params, paramDescs, paramJavaClass, primarys, insParams, updParams,
				objectMap);
	...
	protected void dealExportUrl(List<String> selParams, List<String> selParamJavaClass, List<Integer> selType,
		...
		for (int i = 0, length = selParams.size(); i < length; i++) {
			...
			// 为区间查询时
			if (selType.get(i) == 2) {
				exportParamUrl.add("'" + (i == 0 ? "" : "&") + "START_" + selParams.get(i) + "=' + encodeURIComponent("
						+ (paramIsDate
								? "(typeof (formProps.form?.getFieldValue('" + selParams.get(i)
										+ "')[0]) == 'string') ? formProps.form?.getFieldValue('" + selParams.get(i)
										+ "')[0] : formProps.form?.getFieldValue('" + selParams.get(i)
										+ "')[0].format('YYYY-MM-DD')"
								: "formProps.form?.getFieldValue('" + selParams.get(i) + "')[0]")
						+ ")");
				exportParamUrl.add("'&END_" + selParams.get(i) + "=' + encodeURIComponent("
						+ (paramIsDate
								? "(typeof (formProps.form?.getFieldValue('" + selParams.get(i)
										+ "')[1]) == 'string') ? formProps.form?.getFieldValue('" + selParams.get(i)
										+ "')[1] : formProps.form?.getFieldValue('" + selParams.get(i)
										+ "')[1].format('YYYY-MM-DD')"
								: "formProps.form?.getFieldValue('" + selParams.get(i) + "')[1]")
						+ ")");
			}
			else {
				exportParamUrl.add("'" + (i == 0 ? "" : "&") + selParams.get(i) + "=' + encodeURIComponent("
						+ (paramIsDate ? "(typeof (formProps.form?.getFieldValue('" + selParams.get(i)
								+ "')) == 'string') ? formProps.form?.getFieldValue('" + selParams.get(i)
								+ "') : formProps.form?.getFieldValue('" + selParams.get(i) + "').format('YYYY-MM-DD')"
								: "formProps.form?.getFieldValue('" + selParams.get(i)
										+ "') ? formProps.form?.getFieldValue('" + selParams.get(i) + "') : ''")
						+ ")");
			}
		}
	 ...
	 * @param selType 查询类型
	 */
	private void dealTableParam(StringJoiner tableParams, List<String> params, List<String> paramDescs,
			List<String> paramJavaClass, List<String> primarys, List<String> selParams, List<Integer> selType) {
			...
			if (position > -1 && selType.get(position) == 2) {
				...
				if (!paramIsDate && !paramIsDigit) {
					tableParams.add("{\n" + "      title: '开始" + paramDescs.get(i) + "',\n" + "      dataIndex: 'START_"
							...
							+ (paramIsDate
									? "      initialValue: moment(moment().year() + '-12-31').format('YYYY-MM-DD') //设置默认值\n"
									: "")
							+ "    }");
				}
				else {
					...
					tableParams.add("{\n" + "      title: '" + paramDescs.get(i) + "',\n" + "      dataIndex: '"
							...
							+ "      order: 3,\n" + "      //initialValue: ["
							+ (paramIsDate ? "getMonthFirstDay(), getDate()" : "1, 10") + "],\n" + "      search: {\n"
							+ "        transform: (value) => {\n" + "          return {\n" + "            START_"
							+ params.get(i) + ": value[0],\n" + "            END_" + params.get(i) + ": value[1],\n"
							+ "          };\n" + "        },\n" + "      },\n" + "      hideInTable: true,\n"
							+ "    }");
				}
			}
	 ...
	 * 处理控制层查询中日期类型区间查询的处理
	 */
	private void dealSelControllerDate(List<String> selParams, List<String> selParamJavaClass, List<Integer> selType,
		...
		for (int i = 0, length = selParams.size(); i < length; i++) {
			...
			if (paramIsDate && selType.get(i) == 2) {
				selControllerDate.add(
						"        END_" + selParams.get(i) + " = BaseUtils.addOneDay(END_" + selParams.get(i) + ");");
			}
		}
	 ...
	 */
	private void dealUpdateForm(StringBuilder updateForm, String moduleName, List<String> params,
			List<String> paramDescs, List<String> paramJavaClass, List<String> primarys, List<String> insParams,
			List<String> updParams, Map<String, Object> objectMap) {
		...
		for (int i = 0, length = params.size(); i < length; i++) {
			...
			if (paramIsIns || paramIsUpd) {
				updateForm.append("        <" + getProFormType(javaClass) + "\n" + "          label=\""
						...
						+ getProFormOther(javaClass, params.get(i), antd_initForm) + "          rules={[\n"
						+ "            {\n" + "              required: false,\n" + "              message: '"
						+ paramDescs.get(i) + "为必填项'\n" + "            }\n" + "          ]}\n" + "        />\n");
			}
			// 非新增非修改时对布局的处理
	 ...
	 * @param primarys 主键参数
	 */
	private void dealView1Form(StringJoiner viewForm, StringJoiner dataSourceType, String moduleName,
			List<String> params, List<String> paramDescs, List<String> paramJavaClass, List<String> primarys,
			boolean hasView) {
		if (!hasView) {
			return;
		...
		for (int i = 0, length = params.size(); i < length; i++) {
			...
			viewForm.add("<Descriptions.Item label=\"" + paramDescs.get(i) + "\">{"
					+ BaseUtils.toLowerCase4Index(moduleName) + "." + params.get(i) + "}</Descriptions.Item>");
		}
	}
	 ...
	 * @param primarys 主键参数
	 */
	private void dealViewForm(StringJoiner viewForm, List<String> params, List<String> paramDescs,
			List<String> paramJavaClass, List<String> primarys, boolean hasView) {
		if (!hasView) {
			return;
		}
		for (int i = 0, length = params.size(); i < length; i++) {
