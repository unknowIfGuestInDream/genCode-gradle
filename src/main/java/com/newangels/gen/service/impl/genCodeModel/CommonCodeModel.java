...
public class CommonCodeModel extends AbstractGenCodeModel {
		...
		for (int i = 0, length = selParams.size(); i < length; i++) {
			...
			if (selType.get(i) == 2) {
				exportParamUrl.add("'" + (i == 0 ? "" : "&") + "START_" + selParams.get(i)
						+ "=' + encodeURIComponent(Ext.getCmp(\"START_" + selParams.get(i) + "\")."
						+ (paramIsDate ? "getSubmitValue()" : "getValue()") + ")");
				exportParamUrl.add("'&END_" + selParams.get(i) + "=' + encodeURIComponent(Ext.getCmp(\"END_"
	 ...
	 * @param primarys 主键参数
	 */
	private void dealStoreParam(StringJoiner storeParams, StringJoiner gridParams, List<String> params,
			List<String> paramDescs, List<String> paramJavaClass, List<String> primarys) {
		for (int i = 0, length = params.size(); i < length; i++) {
			...
			gridParams.add("{\n" + "                text: '" + paramDescs.get(i) + "',\n"
					...
					+ ((javaClass == JavaClass.Integer || javaClass == JavaClass.Double)
							? "                align: 'right',\n" : "")
					+ "                flex: 1,\n" + "                minWidth: 150"
	 ...
	 */
	private void dealSelFormAndParam(StringJoiner selForm, StringJoiner selExtraParams, String moduleName,
			List<String> selParams, List<String> selParamDescs, List<String> selParamJavaClass, List<Integer> selType) {
		for (int i = 0, length = selParams.size(); i < length; i++) {
			// java类型为date的前台获取值的方法为getSubmitValue
			JavaClass javaClass = JavaClass.fromCode(selParamJavaClass.get(i));
			boolean paramIsDate = javaClass == JavaClass.Date;
			// 为区间查询
			if (selType.get(i) == 2) {
				selExtraParams.add("'START_" + selParams.get(i) + "': Ext.getCmp('START_" + selParams.get(i) + "')."
						+ (paramIsDate ? "getSubmitValue()" : "getValue()"));
				selExtraParams.add("'END_" + selParams.get(i) + "': Ext.getCmp('END_" + selParams.get(i) + "')."
						+ (paramIsDate ? "getSubmitValue()" : "getValue()"));
				selForm.add("{\n" + "                xtype: '" + getFormXtype(javaClass) + "',\n"
						...
						+ "                        Ext.getCmp('END_" + selParams.get(i)
						+ "').setMinValue(Ext.getCmp('START_" + selParams.get(i) + "').getValue());"
						+ "                    },\n" + "                }\n" + "            }");
				selForm.add("{\n" + "                xtype: '" + getFormXtype(javaClass) + "',\n"
						...
						+ "                            _select" + moduleName + "();\n" + "                        }\n"
						+ "                    },\n" + "                    change: function (combo, records) {\n"
						+ "                        Ext.getCmp('START_" + selParams.get(i)
						+ "').setMaxValue(Ext.getCmp('END_" + selParams.get(i) + "').getValue());"
						+ "                    },\n" + "                }\n" + "            }");
			}
			else {
				selExtraParams.add("'" + selParams.get(i) + "': Ext.getCmp('" + selParams.get(i) + "')."
						+ (paramIsDate ? "getSubmitValue()" : "getValue()"));
				selForm.add("{\n" + "                xtype: '" + getFormXtype(javaClass) + "',\n"
						+ "                id: '" + selParams.get(i) + "',\n" + "                fieldLabel: '"
						+ selParamDescs.get(i) + "',\n" + "                listeners: {\n"
	 ...
	 * @param primarys 主键参数
	 */
	private void dealInsFormPanel(StringJoiner insForm, List<String> insParams, List<String> insParamDescs,
			List<String> insParamJavaClass, List<String> primarys) {
		for (int i = 0, length = insParams.size(); i < length; i++) {
			// 主键不显示在formPanel中
			if (primarys.contains(insParams.get(i))) {
				continue;
			}
			insForm.add("{\n" + getExtForm(insParamJavaClass.get(i)) + "                name: '" + insParams.get(i)
	 ...
	 * @param primarys 主键参数
	 */
	private void dealUpdFormPanel(StringJoiner updForm, List<String> updParams, List<String> updParamDescs,
			List<String> updParamJavaClass, List<String> primarys) {
		...
		for (int i = 0, length = updParams.size(); i < length; i++) {
			updForm.add("{\n" + getExtForm(updParamJavaClass.get(i)) + "                name: '" + updParams.get(i)
	 ...
	 * @param primarys 主键参数
	 */
	private void dealViewFormPanel(StringJoiner viewForm, List<String> params, List<String> paramDescs,
	 ...
	 */
	private void dealControllerPageHandler(String module, String moduleDesc, String packageName,
		...
		String pageHander = "\n    /**\n" + "     * " + moduleDesc + "管理页\n" + "     */\n" + "    @GetMapping(\"/manage"
				...
				+ "() {\n" + "        return new ModelAndView(\"pages/" + frontPackage + "/" + lowModule + "/preInsert"
				+ module + "\");\n" + "    }\n" + "\n" + "    /**\n" + "     * " + moduleDesc + "修改页\n" + "     */\n"
				+ "    @GetMapping(\"/preUpdate" + module + "\")\n" + "    public ModelAndView preUpdate" + module
				+ "() {\n" + "        return new ModelAndView(\"pages/" + frontPackage + "/" + lowModule + "/preUpdate"
				+ module + "\");\n" + "    }\n";
	...
	protected List<String> getTabList(Map<String, Object> objectMap) {
		String module = objectMap.get("module").toString();
		return new ArrayList<>(Arrays.asList("controller", "service", "serviceImpl", "manage" + module + ".html",
				"preInsert" + module + ".html", "preUpdate" + module + ".html", "view" + module + ".html", "base.js",
				"BaseUtils", "BaseSqlCriteria"));
	}
